...
* 梯度方向由 {{< math >}}$ \nabla_\theta \log \pi ${{< /math >}} 决定；
* 学习信号由优势函数 {{< math >}}$A${{< /math >}} 提供；
* 它本质上是 **on-policy** 的结论。

但工程上我们并不是直接“写梯度”，而是希望：

> 构造一个目标函数 {{< math >}}$L(\theta)${{< /math >}}，使其梯度自动给出合理的策略更新。
...
* rollout 使用的是旧策略 {{< math >}}$ \pi_{\text{old}} ${{< /math >}}；
* 更新时参数已经变为 {{< math >}}$ \pi_\theta ${{< /math >}}。
...
* 当 {{< math >}}$ \hat A_t > 0 ${{< /math >}} 时，优化会推动 {{< math >}}$ r_t \to +\infty ${{< /math >}}；
* 当 {{< math >}}$ \hat A_t < 0 ${{< /math >}} 时，优化会推动 {{< math >}}$ r_t \to 0 ${{< /math >}}；
* {{< math >}}$ r_t ${{< /math >}} 对参数的依赖是指数型的（通过 log-prob）。

结论是：

> {{< math >}}$L^{\text{PG-IS}}${{< /math >}} 在一阶近似上是正确的，但在数值优化中是**高度不受控的**。
...
> {{< math >}}$r_t${{< /math >}} 是为了“数据复用”而付出的代价，而 Clip 是对这个代价的安全约束。
