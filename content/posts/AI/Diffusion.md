...
现在你引入一个偏好（loss / reward / constraint），定义一个新目标分布：
...
这是一个非常标准的概率建模操作（能量模型、贝叶斯后验、Boltzmann 分布）。
...
- $\nabla_x \log p(x)$：原 score  
- $\lambda \nabla_x R(x)$：loss / reward 梯度
...
这就是为什么梯度可以**直接加在 score 上**。  
不是 heuristic，而是**目标分布改变后的精确结果**。

---

# 三、那 diffusion 的反向采样在用什么？
...
- $\nabla R(\hat{x}_0)$：把偏好通过“当前估计的 clean sample”传回到 $x_t$ 的引导项

因此：  
**不是在“乱改一步采样”，而是在对新的目标分布做合法的反向扩散。**

## Q1：也就是说，我可以通过基分布 $p(x)$，再通过某些引导（guidance），生成一个新的分布吗？

**A：是的——但必须加上一句非常关键的限定，才是完全正确的说法。**
...
$
是的：你可以从一个基分布 \; p(x) \; 出发，  
通过在生成过程中加入引导（guidance），  
得到一个“新的、被偏置过的生成分布”。  

但这个新分布通常是：  
被定义在“生成过程 / 路径”上的近似分布，  
而不一定等价于一个事先写好的、严格的闭式分布。
$
{{< /math >}}
...
## Q2：那“通过引导得到新分布”在理论上到底有几种不同层级的含义？

**A：可以清楚地分成三种层级，你现在问的其实是在这三者之间切换。**


### ✅ 情形 A：严格成立，有理论保证（理想化情形）
...
### ⚠️ 情形 B：真实 diffusion / guided diffusion 的情况

现实中发生了这些变化：

- $p(x)$ 不是直接建模，而是通过多尺度 $p_t(x)$  
- score 由神经网络近似  
- reward 通常在去噪估计  
  $\hat{x}_0(x_t)$  
  上计算  
...
这是 **principled approximation（有原则的近似）**，  
但不是严格后验采样器。


### 🧭 情形 C：控制 / 推断视角（实践中最稳妥的理解）
...
## Q3：那我到底该怎么一句话总结，才不会再混乱？

**A：你可以安全地使用下面这句话作为最终结论。**

{{< math >}}
$
是的，我可以从 \; p(x) \; 出发，通过引导，生成一个新的分布；  
只是这个“新分布”的严格含义，  
取决于我是在理想化的 Langevin 情形下，  
还是在实际的 diffusion 生成过程中。
$
{{< /math >}}

只要你记住这一点：

- 你就不会把 guided diffusion 误当成“精确后验采样”
- 也不会低估它在生成与控制中的理论正当性
